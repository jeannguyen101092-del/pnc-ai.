...
# ================= 3. SCRAPER (FIXED POSITION 0.3-0.2) =================
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_content)) as pdf:
            for page in pdf.pages:
                ...
                # Tinh chỉnh để bắt đúng dòng, không bị lệch thông số
...
if mode == "🔍 Audit Mode":
    ...
    if f_audit:
        ...
        if target and target['img']:
            ...
            if sel:
                ...
                st.download_button("📥 Xuất Audit Excel", to_excel(audit_dfs, sheet_names), f"Audit_{sel['file_name']}.xlsx")

elif mode == "🔄 Version Control":
    st.subheader("🔄 So sánh 2 file PDF mới (Quét toàn bộ trang)")
    c1, c2 = st.columns(2)
    ...
    if f1 and f2:
        if st.button("⚡ Bắt đầu so sánh toàn diện", use_container_width=True):
            # Hàm bổ trợ quét toàn bộ trang thông số
            def extract_full_pdf(file_content):
                all_specs = {}
                POM_KWS = ["WAIST", "HIP", "THIGH", "KNEE", "LEG", "INSEAM", "RISE", "LENGTH", "CHEST", "SHOULDER", "POM", "SPEC"]
                try:
                    # 1. Lấy ảnh trang đầu làm đại diện
                    doc = fitz.open(stream=file_content, filetype="pdf")
                    pix = doc.load_page(0).get_pixmap(matrix=fitz.Matrix(2.0, 2.0))
                    img_bytes = io.BytesIO()
                    Image.open(io.BytesIO(pix.tobytes("png"))).save(img_bytes, format="WEBP", quality=70)
                    img_data = img_bytes.getvalue()
                    doc.close()

                    # 2. Quét TẤT CẢ các trang để tìm thông số
                    with pdfplumber.open(io.BytesIO(file_content)) as pdf:
                        for page in pdf.pages:
                            words = page.extract_words()
                            if not words: continue
                            ...
                            for y, group in df_w.groupby('y_grid'):
                                ...
                                # Nếu dòng chứa từ khóa thông số
                                if any(kw in line_txt.upper() for kw in POM_KWS) and line_vals:
                                    pom_name = re.sub(r'[\d./\s]+$', '', line_txt).strip()
                                    if len(pom_name) > 2:
                                        for i, val in enumerate(line_vals):
                                            s_key = f"Size_{i+1}"
                                            if s_key not in all_specs: all_specs[s_key] = {}
                                            # Nếu trùng tên điểm đo ở trang sau, vẫn cập nhật hoặc giữ nguyên
                                            all_specs[s_key][pom_name] = val
                    return {"all_specs": all_specs, "img": img_data}
                except: return None

            with st.spinner("Đang quét toàn bộ các trang PDF..."):
                d1, d2 = extract_full_pdf(f1.getvalue()), extract_full_pdf(f2.getvalue())
            
            if d1 and d2:
                st.divider()
                col_a, col_b = st.columns(2)
                col_a.image(d1['img'], caption="Ver A (Trang 1)", use_container_width=True)
                col_b.image(d2['img'], caption="Ver B (Trang 1)", use_container_width=True)
                
                # Lấy danh sách Size tổng hợp từ tất cả các trang đã quét
                ...
                for sz in all_sz:
                    with st.expander(f"DANH SÁCH THÔNG SỐ TỔNG HỢP - {sz}", expanded=True):
                        ...
                        # Lấy tất cả điểm đo từ tất cả các trang
                        ...
                        ver_sheets.append(sz)
                
                st.download_button("📥 Xuất Excel Toàn Bộ Thông Số", 
                                 to_excel(version_dfs, ver_sheets), 
                                 "Full_Version_Comparison.xlsx")
