...
supabase = create_client(URL, KEY)import streamlit as st
import io, fitz, pdfplumber, re, pandas as pd, numpy as np
import torch, hashlib, time, uuid
from PIL import Image
from torchvision import models, transforms
...
KEY = "YOUR_SUPABASE_KEY" # <--- THAY KEY CỦA BẠN VÀO ĐÂY
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_content)) as pdf:
            for page in pdf.pages:
                ...
                # BƯỚC 1: Tìm tọa độ X của các cột Size dựa trên dòng tiêu đề
                size_columns = []
                for y, group in df_w.groupby('y_grid'):
                    line_txt = " ".join(group.sort_values('x0')['text']).lower()
                    # Nếu dòng này chứa từ khóa tiêu đề bảng
                    if "size" in line_txt or ("tol" in line_txt and any(re.match(r'\d+', w) for w in line_txt.split())):
                        for _, row in group.iterrows():
                            txt = row['text'].strip().lower()
                            # Kiểm tra nếu text trong ô khớp với định dạng Size (Bỏ qua TOL, +, -)
                            if re.match(SIZE_PATTERN, txt) and txt not in ["tol", "um", "(+)", "(-)"]:
                                size_columns.append({
                                    "size_name": txt.upper(),
                                    "x0": row['x0'] - 5, # Nới lỏng vùng chọn một chút
                                    "x1": row['x1'] + 5
                                })
                        if size_columns: break # Tìm thấy dòng tiêu đề rồi thì thôi

                # BƯỚC 2: Quét thông số dựa trên tọa độ cột đã tìm thấy
                for y, group in df_w.groupby('y_grid'):
                    ...
                    if any(kw in line_txt.upper() for kw in POM_KWS):
                        ...
                        if len(pom_name) > 3:
                            for col in size_columns:
                                # Chỉ lấy chữ nằm trong khoảng X của cột Size này
                                cell_words = sorted_group[(sorted_group['x0'] >= col['x0']) & (sorted_group['x1'] <= col['x1'])]
                                if not cell_words.empty:
                                    val = parse_val(" ".join(cell_words['text']))
                                    if val > 0:
                                        sz_key = col['size_name']
                                        if sz_key not in all_specs: all_specs[sz_key] = {}
                                        all_specs[sz_key][pom_name] = val
...
if mode == "🔍 Audit Mode":
    ...
    if f_audit:
        ...
        if target and target['img']:
            ...
            if sel:
                ...
                # Lấy danh sách size từ file Target
                for sz, t_specs in target['all_specs'].items():
                    with st.expander(f"SIZE: {sz}", expanded=True):
                        # Tìm size tương ứng trong file Ref (so khớp mờ)
    ...
    if st.session_state.get('ver_results'):
        ...
        # Sắp xếp Size theo thứ tự tự nhiên (2, 3, 4...) thay vì chữ cái
