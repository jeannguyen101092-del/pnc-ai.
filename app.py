...
KEY = "DÁN_MÃ_SUPABASE_KEY_CỦA_BẠN_VÀO_ĐÂY" 
...
def get_vector(img_bytes):
    ...
    try:
        ...
        # Cắt bỏ lề nhiễu, tập trung vào Sketch giữa trang (Bỏ text bảng biểu)
        w, h = img.size
        img = img.crop((w*0.15, h*0.1, w*0.85, h*0.6)) 
        
        # Tăng tương phản mạnh để AI chỉ thấy nét vẽ, không thấy chữ mờ
    ...
    if f_audit:
        ...
        if target and target['img']:
            ...
            if res.data:
                ...
                for row in res.data:
                    if row['vector']:
                        score = cosine_similarity(t_vec, np.array(row['vector']).reshape(1, -1))[0][0]
                ...
                with col1:
                    st.image(target['img'], caption="Target Sketch (Đã xử lý AI)", use_container_width=True)
                with col2:
...
if st.session_state['sel_audit']:
    ...
    if common_sz:
        ...
        st.dataframe(report.style.highlight_between(left=-0.25, right=0.25, subset=['Diff'], color='#C2FFD8', inclusive='neither'), use_container_width=True)
    else: st.error("Không tìm thấy Size tương ứng.")
