...
def get_vector(img_bytes):
    try:
        ...
        w, h = img.size
        # Crop để tập trung vào hình vẽ, tránh rác văn bản
...
if mode == "Audit Mode":
    ...
    target_file = st.file_uploader("Upload bản vẽ cần tìm", type=['pdf'], key=f"aud_{st.session_state['up_key']}")
    if target_file:
        with st.spinner("Đang tìm kiếm mẫu giống nhất..."):
            ...
            if t_data and t_data['img']:
                ...
                else: st.warning("Kho dữ liệu trống.")
    ...
    if f1 and f2:
        if st.button("⚡ So sánh ngay", use_container_width=True):
            with st.spinner("Đang quét..."):
                ...
                if d1 and d2: st.session_state['ver_results'] = {"d1": d1, "d2": d2}
    ...
    if st.session_state.get('ver_results'):
        ...
        if version_dfs:
            st.download_button("📥 Xuất Excel", to_excel(version_dfs, ver_sheets), "Compare.xlsx", use_container_width=True)
