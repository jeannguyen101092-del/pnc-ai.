...
# ================= 1. CONFIGURATION & LOGO PPJ (BASE64) =================
...
if 'm_key' not in st.session_state: st.session_state['m_key'] = 0
if 'a_key' not in st.session_state: st.session_state['a_key'] = 0
                    ...
                    for r_idx in range(min(15, len(df))):
                        ...
                        if desc_col != -1:
                            for i, v in enumerate(row):
                                ...
                                if v == "NEW" or len(v) <= 8 or v.isdigit() or v in ["XS","S","M","L","XL"]: size_cols[i] = v
                            if size_cols: break
                    if size_cols:
...
def fuzzy_match(query, choices):
    query_n = re.sub(r'[^a-z0-9]', '', query.lower())
    best_m, max_s = None, 0
    for c in choices:
        c_n = re.sub(r'[^a-z0-9]', '', c.lower())
        if query_n == c_n: return c
        score = difflib.SequenceMatcher(None, query_n, c_n).ratio()
...
with st.sidebar:
    ...
    st.metric("Total Active SKUs", f"{count}")
    
    st.divider()
    new_files = st.file_uploader("📥 Ingest Master Data", accept_multiple_files=True, key=f"up_{st.session_state['m_key']}")
...
file_audit = st.file_uploader("📤 Drag & Drop Tech-Pack for Auditing", type="pdf", key=f"audit_{st.session_state['a_key']}")
...
if file_audit:
    ...
    if target:
        ...
        if res.data:
            ...
            cols = st.columns(4)
            cols[0].image(target['img'], caption="SOURCE FILE", use_container_width=True)
            ...
            if not spec_ref:
                spec_ref = {}
                for s_data in best['spec_json'].values(): spec_ref.update(s_data)
                st.warning(f"⚠️ Size '{sel_size}' fallback activated.")

            report = []
            repo_choices = list(spec_ref.keys())
            for d, v in spec_audit.items():
                m_name = fuzzy_match(d, repo_choices)
                rv = spec_ref[m_name] if m_name else 0
                diff = round(v - rv, 3)
                status = "✅ PASS" if (rv > 0 and abs(v-rv) < 0.2) else "❌ FAIL"
            ...
            col1.download_button("📥 DOWNLOAD XLS REPORT", data=towrite.getvalue(), file_name=f"PPJ_Audit_{file_audit.name}.xlsx", use_container_width=True)
            if col2.button("RESET SESSION", use_container_width=True):
                st.session_state['a_key'] += 1; st.session_state['sel_sku'] = None; st.rerun()
    else: st.error("⚠️ Invalid File.")
