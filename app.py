...
st.set_page_config(layout="wide", page_title="AI Fashion Auditor V48.9", page_icon="🔍")
...
def get_vector(img_bytes):
    ...
    try:
        ...
        # Ép buộc trả về list 512 phần tử
                ...
                for r_idx, row in df.iterrows():
                    ...
                    if any(x in " ".join(row_up) for x in ["POM", "DESCRIPTION"]):
                        for i, v in enumerate(row_up):
                            if "POM" in v or "DESC" in v: n_col = i; break
                        for i, v in enumerate(row_up):
                        ...
                        if n_col != -1 and v_col != -1:
                            for d_idx in range(r_idx + 1, len(df)):
                                d_row = df.iloc[d_idx]
                                p_name = str(d_row[n_col]).strip().upper()
...
with st.sidebar:
    ...
    if st.button("🧹 Dọn dẹp mẫu lỗi"):
        supabase.table("ai_data").delete().neq("id", 0).execute() # Xóa sạch để nạp lại chuẩn
        st.success("Đã xóa sạch Database. Hãy nạp lại mẫu chuẩn!")
        st.rerun()
...
# ================= MAIN =================
st.title("🔍 AI Fashion Auditor V48.9")
...
if data_lib:
    ...
    st.warning("⚠️ Kho đang trống. Hãy nạp mẫu ở Sidebar.")
    ...
    if target["specs"]:
        ...
        if v_test and data_lib:
            ...
            if matches:
                ...
                c2.image(best['item']['image_url'], caption="Mẫu gốc đối chiếu")
                ...
                for k, v in target["specs"].items():
                    ...
                    diff_rows.append({"Hạng mục": k, "Kiểm tra": v, "Gốc": v_ref, "Lệch": diff, "Kết quả": "✅ OK" if abs(diff) < 0.1 else "❌ FAIL"})
                st.table(pd.DataFrame(diff_rows))
            else:
                st.error("❌ Lỗi so khớp: Vector trong kho không chuẩn. Hãy nhấn 'Dọn dẹp' và nạp lại.")
