        ...
        try:
            with pdfplumber.open(io.BytesIO(content)) as pdf:
                for page in pdf.pages:
                    ...
                    size_lanes = []
                    # Danh sách size bao gồm cả size số 0, 2, 4... cho Express
    ...
    if f1 and f2:
        if st.button("⚡ RUN COMPREHENSIVE COMPARISON", use_container_width=True):
            with st.spinner("Analyzing files..."):
                d1 = get_specs_v14(f1.getvalue())
                d2 = get_specs_v14(f2.getvalue())
            
            if d1 and d2:
                # Sắp xếp size số cho đúng thứ tự 0, 2, 4...
                all_sz = sorted(list(set(d1.keys()) | set(d2.keys())), key=lambda x: int(x) if x.isdigit() else 999)
