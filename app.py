...
def extract_data(file_content, scan_all=False):
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_content)) as pdf:
            # Nếu scan_all=True (Version Control) quét hết, ngược lại chỉ quét trang 1 (Nạp kho)
...
if mode == "🔍 Audit Mode":
    ...
    c1, c2 = st.columns(2)
    with c1:
        f_a = st.file_uploader("Upload Version A (Old):", type="pdf", key="v_a")
    with c2:
        f_b = st.file_uploader("Upload Version B (New):", type="pdf", key="v_b")

    if f_a and f_b:
        if st.button("RUN COMPARISON (ALL PAGES)", use_container_width=True):
            # So sánh 2 file mới: Quét TOÀN BỘ trang (scan_all=True)
            ...
            if d_a and d_b:
                ...
                for sz, specs_b in d_b['all_specs'].items():
                    with st.expander(f"SIZE: {sz}", expanded=True):
                        ...
                        st.table(pd.DataFrame(rows).style.format({"Diff": "{:+.2f}"}))
                st.download_button("📥 Tải báo cáo Excel", to_excel(pd.DataFrame(results)), "version_comparison.xlsx")
