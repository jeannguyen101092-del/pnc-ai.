    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(pdf_content)) as pdf:
            for page in pdf.pages:
                ...
                if any(x in text_page for x in ["POLY CORE", "SEWING THREAD", "BUTTON"]): continue
    ...
    if files and st.button("🚀 BẮT ĐẦU NẠP"):
        ...
        for i, f in enumerate(files):
            ...
            if d and d['specs']:
                ...
                # 2. Lệnh nạp cưỡng bức (Thêm bẫy lỗi để nạp tối giản nếu DB thiếu cột)
                try:
                    ...
                    try:
                        ...
                        st.warning(f"Đã nạp tối giản: {f.name}")
...
# --- MAIN: ĐỐI SOÁT ---
st.title("🔍 AI Fashion Auditor V46.2")
    ...
    if target and target['specs']:
        ...
        if db_res.data:
            ...
            for i in db_res.data:
                sim_val = 0
                if v_test is not None and i.get('vector'):
            ...
            for m in top:
                ...
                c1, c2 = st.columns(2)
                # FIX LỖI SYNTAX TẠI ĐÂY (Tách dòng if và with)
                if target['img']:
                    with c1:
                        st.image(target['img'], caption="Bản vẽ đang kiểm")
                if m['data'].get('image_url'):
                    with c2:
                        st.image(m['data']['image_url'], caption="Mẫu gốc trong kho")
                ...
                if diff_list:
                    ...
                    st.download_button(
                        ...
                        data=out.getvalue(),
                        file_name=f"Audit_{m['data']['file_name']}.xlsx",
