...
def parse_val(t):
    try:
        ...
        if num:
            val = float(num[0])
            return val if 0.1 <= val < 150 else 0 # Chỉ lấy số đo thực tế từ 0.1 trở lên
        return 0
    except: return 0
...
# ================= 3. SCRAPER (OPTIMIZED FOR POSITION) =================
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_content)) as pdf:
            for page in pdf.pages:
                ...
                # Tinh chỉnh y_grid để gom dòng chính xác hơn (tránh lệch 0.3 - 0.2)
                df_w['y_grid'] = (df_w['top']).round(0) 
                for y, group in df_w.groupby('y_grid'):
                    sorted_group = group.sort_values('x0')
                    line_txt = " ".join(sorted_group['text'])
                    
                    # Tìm các giá trị số đo (thường nằm bên phải dòng)
                    ...
                    if any(kw in line_txt.upper() for kw in POM_KWS) and line_vals:
                        # Tên POM: lấy các chữ cái, bỏ qua các số ở cuối
    ...
    if f1 and f2:
        if st.button("⚡ Bắt đầu so sánh", use_container_width=True):
            ...
            if d1 and d2:
                ...
                st.download_button("📥 Xuất Excel", to_excel(version_dfs, ver_sheets), "Comparison.xlsx")
