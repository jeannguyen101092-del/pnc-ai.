...
st.set_page_config(layout="wide", page_title="AI Fashion Auditor V95", page_icon="🛡️")
...
def detect_category(text, filename=""):
    ...
    # Jacket/Outerwear/Coat luôn là ÁO
    if any(x in t for x in ["SHIRT", "TOP", "TEE", "JACKET", "HOODIE", "COAT", "OUTERWEAR", "ÁO"]): return "ÁO"
...
def parse_val(t):
    try:
        ...
        txt = txt.replace('⁄', '/') # Sửa lỗi ký tự phân số lạ
...
# ================= 3. TRÍCH XUẤT CẢI TIẾN V95 =================
def is_measurement_table(df):
    """SỬA LỖI applymap -> map cho Pandas mới"""
    text_blob = " ".join(df.astype(str).values.flatten()).upper()
    keywords = ["WAIST", "CHEST", "HIP", "SLEEVE", "LENGTH", "SHOULDER", "SPEC", "POM"]
    kw_score = sum(1 for k in keywords if k in text_blob)
    
    # Đếm số lượng ô có chứa giá trị số
    total_cells = df.size
    # FIX Pandas 2.1+: applymap đổi thành map
    numeric_cells = df.map(lambda x: 1 if parse_val(x) > 0 else 0).values.sum()
    density = numeric_cells / total_cells if total_cells > 0 else 0
    
    return kw_score >= 1 or density > 0.3

def extract_pdf_v95(file, customer="Auto"):
    ...
    try:
        ...
        if len(doc) > 0:
            # FIX: Thử chụp ảnh trang 1, nếu lỗi thử trang 2
            try:
                img_bytes = doc.load_page(0).get_pixmap(matrix=fitz.Matrix(1.5, 1.5)).tobytes("png")
            except:
                if len(doc) > 1:
                    img_bytes = doc.load_page(1).get_pixmap(matrix=fitz.Matrix(1.5, 1.5)).tobytes("png")
                ...
                for tb in tables:
                    ...
                    for r_idx, row in df.head(30).iterrows():
                        ...
                        if customer == "Reitmans":
                            ...
                            for i, v in enumerate(row_up):
                                if any(target in v for target in ["NEW", "SAMPLE", "SPEC", "32", "34", "M", "S", "L", "XL"]):
        ...
        for f in new_files:
            ...
            with st.spinner(f"Đang nạp {f.name}..."):
                ...
                if not data or not data['specs']:
                    st.error(f"❌ {f.name}: Không tìm thấy bảng thông số."); continue
                
                # Nếu không chụp được ảnh, tạo ảnh rỗng để tránh lỗi
                img_data = data['img'] if data['img'] else Image.new('RGB', (224, 224), color = 'white')
...
with cust_col: cust_choice = st.selectbox("Khách hàng:", ["Auto", "Reitmans"])
with size_col: target_size = st.text_input("Size cần lấy (Để trống lấy mẫu chuẩn):", "")

file_audit = st.file_uploader("📤 Upload file PDF cần kiểm tra", type="pdf", key="audit_main")

if file_audit:
    with st.spinner("Đang trích xuất dữ liệu..."):
        target = extract_pdf_v95(file_audit, customer=cust_choice)
    ...
    if target and target["specs"]:
        ...
        if not same_cat_data:
            st.warning(f"⚠️ Chưa có mẫu thuộc nhóm **{target['category']}** trong kho.")
        else:
            img_t = Image.open(io.BytesIO(target['img'])).convert('RGB') if target['img'] else Image.new('RGB', (224,224), 'white')
            ...
            st.divider()
            sel_match = st.selectbox("🤖 AI gợi ý mẫu tương đồng nhất:", top_matches, format_func=lambda x: f"{x['item']['file_name']} (Độ khớp: {x['score']*100:.1f}%)")
            best = sel_match['item']

            c1, c2 = st.columns(2)
            with c1:
                st.markdown("### 📄 ĐANG KIỂM")
                if target['img']: st.image(target["img"], use_container_width=True)
                st.table(pd.DataFrame([{"Vị trí": k, "Số đo": v} for k,v in target["specs"].items()]))
            with c2:
                st.markdown(f"### ✨ MẪU GỐC: {best['file_name']}")
            ...
            st.download_button("📥 TẢI BÁO CÁO EXCEL", output.getvalue(), f"Audit_{best['file_name']}.xlsx", type="primary")
    else:
        st.error("❌ Không thể trích xuất thông số. Hãy kiểm tra PDF của bạn có phải là dạng Scan ảnh không?")
