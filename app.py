...
# ================= 3. PDF EXTRACTION (STRICT FILTER) =================
def extract_pdf_multi_size(file_content):
    ...
    try:
        ...
        # 2. Trích xuất ảnh (Nếu PDF lỗi không có ảnh -> Trả về None)
        try:
        ...
        # LOẠI BỎ FILE NẾU THIẾU HÌNH HOẶC THIẾU THÔNG SỐ
        if not all_specs or img_bytes is None:
            return None
...
with st.sidebar:
    ...
    st.divider()
    st.subheader("📥 Master Data Ingestion")
    ...
    if new_files and st.button("SYNCHRONIZE", use_container_width=True):
        ...
        with st.spinner("Processing..."):
            for f in new_files:
                ...
                # 2. Trích xuất và kiểm tra nội dung (Hình + Số)
                data = extract_pdf_multi_size(c)
                if not data:
                    rejected += 1
                    st.error(f"❌ REJECTED: '{f.name}' has no image or measurement specs.")
                    continue
            ...
            st.session_state['master_reset_key'] += 1
            st.success(f"✅ Processed: {added} Added, {skipped} Duplicates, {rejected} Invalid Files Rejected.")
            st.rerun()
...
if file_audit:
    ...
    if target and target["all_specs"]:
        ...
        if res.data:
            ...
            best = st.session_state['selected_sku'] if st.session_state['selected_sku'] else top_3.iloc.to_dict()
        ...
        st.error("⚠️ REJECTED: This PDF is missing either a clear image or a valid measurement table. Scanning aborted.")
