...
# Khởi tạo bộ nhớ tạm để không bị mất dữ liệu khi chuyển trang hoặc nhấn nút
            ...
            for page in pdf.pages:
                ...
                for y, group in df_w.groupby('y_grid'):
                    ...
                    if any(kw in line_txt.upper() for kw in POM_KWS):
                        ...
                        # Lấy số đo ở khu vực bên phải (x0 > 380 để tránh cột TOL nhưng không mất số đo)
                        val_words = sorted_group[sorted_group['x0'] > 380]['text'].tolist()
...
if mode == "🔍 Audit Mode":
    ...
    if f1 and f2:
        if st.button("⚡ Bắt đầu so sánh toàn diện", use_container_width=True):
            with st.spinner("Đang trích xuất dữ liệu toàn bộ trang..."):
                d1 = extract_full_data(f1.getvalue())
                d2 = extract_full_data(f2.getvalue())
                if d1 and d2:
                    # Lưu kết quả vào session_state để không bị mất khi nhấn download
    ...
    if st.session_state['ver_results']:
        ...
        if st.button("Xóa kết quả để so sánh file khác"):
