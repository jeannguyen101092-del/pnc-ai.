...
st.set_page_config(layout="wide", page_title="AI FASHION AUDITOR V34.9", page_icon="📊")
...
# ================= MAIN =================
st.title("🔍 AI FASHION AUDITOR V34.9")
        ...
        if samples and t_vec:
            ...
            with col2:
                ...
                if test_data['tables'] and ref_data['spec_json']:
                    ...
                    # FIX DÒNG 135: Đảm bảo danh sách noise đầy đủ và đóng ngoặc
                    noise =
                    ...
                    if actual_sizes:
                        ...
                        if not df_ref.empty and sel_size:
                            for _, row_t in df_test.iterrows():
                                ...
                                if not match.empty:
                                    try:
                                        ...
                                        diff = round(v1 - v2, 2)
                                        audit_data.append({"Hạng mục": desc, f"Kiểm ({sel_size})": v1, f"Gốc ({sel_size})": v2, "Lệch": diff, "Kết quả": "✅ OK" if abs(diff) <= 0.5 else "❌ LỆCH"})
                                    except: pass
                            ...
                            if audit_data:
                                ...
                                st.warning("⚠️ Không tìm thấy hạng mục khớp.")
                    else:
