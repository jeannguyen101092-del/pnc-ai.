...
st.set_page_config(layout="wide", page_title="AI FASHION PRO V20.0", page_icon="🛡️")
...
def extract_valid_techpack(pdf_file):
    """Chỉ lấy file có ĐỦ Ảnh và ĐỦ Thông số POM. Thiếu là BỎ QUA."""
    specs, img, text = {}, None, ""
    try:
                ...
                for tb in page.extract_tables():
                    for row in tb:
                        ...
                        if len(row_c) >= 2:
                            key = row_c[0].upper()
                            val = row_c[-1]
                            # Lọc POM: Có chữ cái ở key và có số ở giá trị
                            if len(key) > 4 and any(c.isdigit() for c in val):
                                specs[key] = val
        
        # ĐIỀU KIỆN: Phải có ảnh VÀ có ít nhất 3 dòng thông số POM
...
with st.sidebar:
    ...
    st.metric("MẪU HỢP LỆ TRONG KHO", len(samples))
...
st.title("🛡️ AI POM CHECKER V20.0 - SIÊU SO SÁNH")
