    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_content)) as pdf:
            for page in pdf.pages:
                ...
                # BƯỚC 1: Dò tìm vị trí X của các cột Size (Bỏ qua TOL)
                size_cols = []
                ...
                # BƯỚC 2: Trích xuất dữ liệu dựa trên cột đã dò được
                for y, group in df_w.groupby('y_grid'):
                    ...
                    if any(kw in line_txt.upper() for kw in POM_KWS):
                        ...
                        if len(pom_name) > 3:
                            for col in size_cols:
                                # Lấy text nằm đúng trong phạm vi cột Size
...
with st.sidebar:
    ...
    except: st.write("Database connecting...")
    st.divider()
