...
# --- TRÍCH XUẤT THÔNG SỐ (NÉ TRANG BOM) ---
            ...
            for page in pdf.pages:
                ...
                for tb in tables:
                    ...
                    for r_idx, row in df.head(15).iterrows():
                        ...
                        for i, c in enumerate(row_up):
                            if i != p_col and any(k in c for k in ["NEW", "FINAL", "SAMPLE", "SPEC", "SIZE", " M "]): v_col = i
...
with st.sidebar:
    ...
    if files and st.button("🚀 BẮT ĐẦU NẠP"):
        ...
        for idx, f in enumerate(files):
            ...
            if d and d['specs']:
                try:
                    # NẠP THẲNG VÀO DB (Bỏ qua xử lý ảnh gây treo)
...
if t_file:
    ...
    if target and target['specs']:
        ...
        if db_res.data:
            m = db_res.data[-1] # Lấy mẫu cuối cùng nạp vào kho
