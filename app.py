...
# ================= 3. TRÍCH XUẤT ĐA SIZE (NÂNG CẤP CHO DENIM) =================
                ...
                for tb in tables:
                    ...
                    for r_idx, row in df.head(15).iterrows():
                        ...
                        for i, v in enumerate(row_up):
                            ...
                            # Mở rộng nhận diện size số (24-40) và size chữ
                            elif v.isdigit() or any(s == v for s in ["S", "M", "L", "XL", "XS"]): 
                                size_cols[i] = v
...
with st.sidebar:
    ...
    st.divider()
    # Ô NHẬP TÊN KHÁCH HÀNG KHI NẠP FILE
    cust_input_save = st.text_input("Nhập tên khách hàng (để lưu vào kho)", "TP MỚI")
        ...
        for i, f in enumerate(new_files):
            ...
            if data and data['all_specs']:
                ...
                supabase.table("ai_data").insert({
                    ...
                    "customer_name": cust_input_save.upper() # Lấy từ ô nhập tay
                }).execute()
            progress_bar.progress((i + 1) / len(new_files))
        st.success("Nạp thành công!"); st.session_state.up_key += 1; st.rerun()

# ================= 5. ĐỐI SOÁT CHÍNH + NHẬP TÊN KHÁCH AUDIT =================
st.title("🔍 AI SMART AUDITOR V99.3")

# Ô NHẬP TÊN KHÁCH HÀNG KHI AUDIT
...
if file_audit:
    with st.status("🛠️ Đang quét nội dung PDF và đối soát AI...", expanded=True) as status:
        ...
        if target and target["all_specs"]:
            st.write(f"2. Loại hàng nhận diện: **{target['category']}**. Khách: **{cust_audit_input.upper()}**")
            ...
            if res.data:
                ...
                # ƯU TIÊN THEO TÊN KHÁCH HÀNG NHẬP TAY
                ...
                for i, (idx, row) in enumerate(df_top.iterrows()):
                    with tabs[i]:
                        ...
                        with c_info:
                            ...
                            st.write(f"📌 Khách trong kho: **{row['customer_name']}**")
                        
                        with c_table:
                            common_sizes = sorted(list(set(target['all_specs'].keys()) & set(row['spec_json'].keys())))
                            if not common_sizes:
                                # Nếu ko khớp tên size, lấy danh sách size của bản audit để chọn
                            ...
                            if common_sizes:
                                ...
                                # Lấy size tương ứng của mẫu gốc
