...
def parse_val(t):
    try:
        ...
        if not t or any(x in t for x in ["wash", "color", "label", "ticket", "style", "page", "date"]): return 0
        t = t.replace(',', '.')
        ...
        return val if val < 200 else 0 # Chặn các số quá lớn (thường là mã hàng)
    except: return 0

# ================= 3. PDF EXTRACTION (MASTER SCRAPER 5.0 - SPEC HUNTER) =================
def extract_full_techpack(file_content):
    ...
    IGNORE_KWS = ["STYLE ID", "SHORT DESC", "DEPARTMENT", "DATE", "SEASON", "VENDOR", "FABRIC"]
    ...
    try:
        ...
        with pdfplumber.open(io.BytesIO(file_content)) as pdf:
            ...
            for page in pdf.pages:
                ...
                # --- CHIẾN THUẬT QUÉT BẢNG THÔNG MINH ---
                ...
                for tb in tables:
                    ...
                    # 1. Tìm cột Description (Point of Measure)
                    desc_col = -1
                    for c_idx in range(len(df.columns)):
                        col_str = " ".join(df.iloc[:, c_idx].astype(str).upper())
                        if any(kw in col_str for kw in POM_KWS) and not any(ik in col_str for ik in IGNORE_KWS):
                    ...
                    for col_idx in range(len(df.columns)):
                        ...
                        # Kiểm tra xem cột có chứa thông số may mặc thực sự không
                        vals = [parse_val(v) for v in df.iloc[:, col_idx]]
                        if sum(1 for v in vals if 0 < v < 150) >= 2: # Có ít nhất 2 số đo hợp lý
                            # Lấy tên Size (thường ở dòng 0, 1 hoặc 2)
                            s_name = "Unknown"
                            for r_idx in range(min(3, len(df))):
                                raw = str(df.iloc[r_idx, col_idx]).strip().replace('\n', ' ')
                                if raw and len(raw) < 10 and not any(k in raw.upper() for k in ["TOL", "CODE", "SPEC"]):
                                    s_name = raw; break
                            ...
                            for d_idx in range(len(df)):
                                ...
                                # Chỉ lấy nếu là tên POM hợp lệ và không nằm trong danh sách bỏ qua
                                if val > 0 and len(pom) > 3 and not any(ik in pom.upper() for ik in IGNORE_KWS):
...
with st.sidebar:
    ...
    if new_files and st.button("SYNCHRONIZE", use_container_width=True):
        with st.spinner("Spec Hunter Active..."):
            for f in new_files:
                ...
                if data and data.get('all_specs'):
                    ...
                    supabase.table("ai_data").upsert({
                        ...
                        "spec_json": data['all_specs'], "category": data['category'], "image_url": supabase.storage.from_(BUCKET).get_public_url(path)
                    }).execute()
        st.rerun()
...
if mode == "🔍 AI Search (Audit)":
    ...
    if file_audit:
        ...
        # (Giữ nguyên logic tìm tương đồng Top 3...)
        all_db = []
        ...
        if selected:
            ...
            for sz, t_specs in target['all_specs'].items():
                with st.expander(f"SIZE: {sz}", expanded=True):
                    ...
                    for p, v in t_specs.items():
                        ...
                        rows.append({"Point of Measure": p, "Target": v, "Ref": rv, "Diff": f"{v-rv:+.3f}"})
            ...
            if all_ex:
                ...
                st.download_button("📥 DOWNLOAD REPORT", buf.getvalue(), f"Audit.xlsx")
    ...
    if file_b and data_b:
        ...
        if all_r_ex:
            ...
            st.download_button("📥 DOWNLOAD COMPARISON (.XLSX)", buf_r.getvalue(), f"Round_Comparison.xlsx")
