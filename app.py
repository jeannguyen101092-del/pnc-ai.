...
st.set_page_config(layout="wide", page_title="AI Fashion Pro V45.5", page_icon="📊")
...
# --- HÀM PARSE SỐ (CHUẨN REITMANS: PHÂN SỐ & THẬP PHÂN) ---
def parse_val(t):
    try:
            ...
            for page in pdf.pages:
                ...
                for tb in tables:
                    ...
                    for r_idx, row in df.head(15).iterrows():
                        ...
                        # Tìm cột Tên: Ưu tiên POM NAME (Reitmans) hoặc DESCRIPTION (Express)
                        for i, c in enumerate(row_up):
                            if any(k in c for k in ["POM NAME", "DESCRIPTION", "ITEM", "POINT OF MEASURE"]):
                                p_col = i
                                break
                        # Tìm cột Số đo: Ưu tiên NEW (Reitmans) hoặc Size (Express)
                        for i, c in enumerate(row_up):
                            if i != p_col and any(k in c for k in ["NEW", "FINAL", "SAMPLE", "SPEC", " M ", " L ", " 32 "]):
                                v_col = i
                                break
...
if t_file:
    target = extract_pom_v455(t_file)
    if target and target['specs']:
        st.success(f"✅ Tìm thấy {len(target['specs'])} thông số từ file kiểm.")
        db_res = supabase.table("ai_data").select("*").execute()
        if db_res.data:
            # Đối chiếu với mẫu cuối cùng nạp vào kho
            m = db_res.data[-1] 
            st.subheader(f"✨ Đối chiếu với mẫu gốc: {m['file_name']}")
            
            c1, c2 = st.columns(2)
            # FIX LỖI CÚ PHÁP TẠI ĐÂY
            if target['img']:
                with c1:
                    st.image(target['img'], caption="Hình ảnh file đang kiểm")
            if m['image_url']:
                with c2:
                    st.image(m['image_url'], caption="Hình ảnh mẫu gốc trong kho")

            # Xử lý đối chiếu thông số
            diff_list = []
            ...
            if diff_list:
                ...
                st.download_button(
                    ...
                    data=out.getvalue(),
                    file_name=f"Audit_Report_{m['file_name']}.xlsx",
