...
# ================= 5. AUDIT INTERFACE (UNLIMITED DATA) =================
...
if mode == "🔍 AI Search (Audit)":
    ...
    if file_audit:
        # BƯỚC 1: ĐỌC FILE TARGET
        with st.status("Analyzing target file...", expanded=True) as status:
            target = extract_full_techpack(file_audit.read())
            if not target: st.error("Lỗi đọc file PDF!"); st.stop()
            
            # BƯỚC 2: QUÉT KHO (VƯỢT GIỚI HẠN 1000)
            status.update(label=f"Scanning repository ({count} SKUs)...")
            all_db_data = []
            # Lấy theo đợt 1000 dòng để không bị sót dữ liệu
            for i in range(0, count, 1000):
                res = supabase.table("ai_data").select("id, vector, file_name, category").range(i, i + 999).execute()
                all_db_data.extend(res.data)
            df_db = pd.DataFrame(all_db_data)

            # BƯỚC 3: AI MATCHING
            status.update(label="Calculating similarity...")
            ...
            df_db['final'] = df_db['sim'] * df_db.apply(smart_score, axis=1)
