...
st.set_page_config(layout="wide", page_title="AI Fashion Auditor V46.5 PRO", page_icon="🔍")
...
def normalize_pom(t):
    """Làm sạch tên POM để so khớp chính xác dù Brand nào"""
    if not t: return ""
    s = str(t).upper().strip()
    s = re.sub(r'[^A-Z0-9]', '', s) # Chỉ giữ lại chữ và số để khớp tuyệt đối
    return s
...
# --- MAIN: ĐỐI SOÁT ---
st.title("🔍 AI Fashion Auditor V46.5 PRO")
    ...
    if target and target['specs']:
        ...
        if not db.data:
            ...
            for row in db.data:
                ...
                # 🔥 FIX LỖI ÉP KIỂU TẠI ĐÂY: Xử lý dữ liệu vector dù là string hay list
                if v_raw:
                    try:
            ...
            if matches:
                ...
                m = top_matches[0] # Lấy mẫu đầu tiên
                ...
                st.error("Không tìm thấy dữ liệu vector hợp lệ. Hãy nạp lại kho dữ liệu chuẩn!")
