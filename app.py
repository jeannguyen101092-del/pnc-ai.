...
st.set_page_config(layout="wide", page_title="AI Fashion Pro V4.9", page_icon="👔")
...
# --- não PHÂN LOẠI THÔNG MINH (Dựa trên bảng quy tắc của bạn) ---
def advanced_classify(specs, text_content, file_name):
    ...
    return "ÁO" if any(k in txt for k in ['CHEST', 'HPS', 'SHIRT', 'TEE']) else "KHÁC"
...
with st.sidebar:
    ...
    if up_bulk and st.button("🚀 BẮT ĐẦU NẠP"):
        ...
        st.success("Đã nạp kho và phân loại xong!")
        st.rerun()
...
if up_test:
    ...
    if target:
        ...
        # Lấy dữ liệu CÙNG LOẠI để so sánh chính xác
        ...
        if not db.data:
            ...
            # So sánh AI (An toàn - Chặn lỗi đỏ)
            tf = transforms.Compose([transforms.Resize(224), transforms.CenterCrop(224), transforms.ToTensor(), transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])
            with torch.no_grad():
            ...
            if sims:
                ...
                c1, c2 = st.columns(2)
                with c1:
                    st.image(target['img_bytes'], caption="Mẫu mới: " + up_test.name, use_container_width=True)
                with c2:
                    st.image(base64.b64decode(best['img']), caption=f"Khớp nhất: {best['name']} ({best['sim']:.1f}%)", use_container_width=True)
