...
def extract_full_data(file_content, filename=""):
    ...
    try:
        ...
        dna = {
            ...
            "construction": {"closure": "button" if "BUTTON FLY" in text else "zipper"}
        }
        return {"all_specs": all_specs, "img": img_data, "dna": dna, "vector": get_vector(img_data), "name": filename}
    except: return None
        ...
        for i, f in enumerate(new_files):
            ...
            if data:
                ...
                supabase.table("ai_data").upsert({
                    ...
                    "dna_json": data['dna'], "vector": data['vector'], "image_url": supabase.storage.from_(BUCKET).get_public_url(path)
                }).execute()
