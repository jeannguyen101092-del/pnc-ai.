...
st.set_page_config(layout="wide", page_title="AI Fashion Auditor V87", page_icon="🛡️")
...
with st.sidebar:
    ...
    new_files = st.file_uploader("Nạp mẫu gốc vào kho", type="pdf", accept_multiple_files=True, key="up_kho")
...
# ================= 5. MAIN (ĐỐI SOÁT) =================
st.title("🔍 AI SMART AUDITOR - V87")

# ĐÂY LÀ CHỖ UPLOAD FILE MỚI ĐỂ KIỂM TRA
file_audit = st.file_uploader("📤 Kéo thả file PDF cần kiểm tra vào đây", type="pdf", key="audit_main")
...
if file_audit:
    ...
    if target:
        ...
        if not same_cat_data:
            st.warning(f"⚠️ Trong kho chưa có mẫu **{target['category']}** nào để đối chiếu.")
        else:
            # CHẾ ĐỘ CHỌN
            st.divider()
            ...
            if mode == "🤖 Tự động (AI)":
                ...
                sel_sample = sorted_matches[0]['item']
                st.write(f"✅ AI khớp nhất với: **{sel_sample['file_name']}** ({sorted_matches[0]['score']*100:.1f}%)")
            else:
        ...
        st.error("❌ PDF không đủ điều kiện đối soát (Thiếu bảng thông số).")
