...
st.set_page_config(layout="wide", page_title="AI Fashion Auditor V51", page_icon="🔍")
...
# ================= MAIN =================
st.title("🔍 AI Fashion Auditor V51")
...
if audit_file:
    ...
    if target["specs"]:
        ...
        if v_test and data_lib:
            # FIX: Chuyển đổi vector kiểm tra sang Numpy chuẩn
            v_test_np = np.array(v_test).reshape(1, -1).astype(np.float32)
            matches = []
            
            for item in data_lib:
                try:
                    # FIX: Ép kiểu dữ liệu vector từ Database sang Numpy để so khớp
                    v_ref = np.array(item["vector"]).reshape(1, -1).astype(np.float32)
                    if v_test_np.shape == v_ref.shape:
            ...
            if matches:
                ...
                for k, v in target["specs"].items():
                    ...
                    diff_rows.append({"Hạng mục (POM)": k, "Đang kiểm": v, "Mẫu gốc": v_ref, "Chênh lệch": diff, "Kết quả": "✅ OK" if abs(diff) < 0.1 else "❌ FAIL"})
                st.table(pd.DataFrame(diff_rows))
            else:
                st.error("❌ Lỗi so khớp: Mẫu trong kho bị lỗi định dạng Vector. Hãy nhấn 'Dọn dẹp' và nạp lại.")
